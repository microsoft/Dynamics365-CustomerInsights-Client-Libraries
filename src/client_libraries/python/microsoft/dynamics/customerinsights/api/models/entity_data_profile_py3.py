# coding=utf-8
# --------------------------------------------------------------------------
# Code generated by Microsoft (R) AutoRest Code Generator.
# Changes may cause incorrect behavior and will be lost if the code is
# regenerated.
# --------------------------------------------------------------------------

from msrest.serialization import Model


class EntityDataProfile(Model):
    """EntityDataProfile.

    :param qualified_entity_name:
    :type qualified_entity_name: str
    :param row_count:
    :type row_count: long
    :param quarantine_row_count:
    :type quarantine_row_count: long
    :param profiling_date:
    :type profiling_date: datetime
    :param profiled_attributes:
    :type profiled_attributes: str
    """

    _attribute_map = {
        'qualified_entity_name': {'key': 'qualifiedEntityName', 'type': 'str'},
        'row_count': {'key': 'rowCount', 'type': 'long'},
        'quarantine_row_count': {'key': 'quarantineRowCount', 'type': 'long'},
        'profiling_date': {'key': 'profilingDate', 'type': 'iso-8601'},
        'profiled_attributes': {'key': 'profiledAttributes', 'type': 'str'},
    }

    def __init__(self, *, qualified_entity_name: str=None, row_count: int=None, quarantine_row_count: int=None, profiling_date=None, profiled_attributes: str=None, **kwargs) -> None:
        super(EntityDataProfile, self).__init__(**kwargs)
        self.qualified_entity_name = qualified_entity_name
        self.row_count = row_count
        self.quarantine_row_count = quarantine_row_count
        self.profiling_date = profiling_date
        self.profiled_attributes = profiled_attributes
